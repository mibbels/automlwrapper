...
import AutoSklearnWrapper
import AutoKerasWrapper
import AutoGluonWrapper
...
class AutoMLWrapper:
    ...
    def initialize(self, data: Any, target_column: str, task_type: Optional[str] = None, data_type: Optional[str] = None, problem_type: Optional[str] = None) -> None:
        if self.library_name == "auto-sklearn":
            self.library = AutoSklearnWrapper.AutoSklearnWrapper()

        elif self.library_name == "auto-keras":
            self.library = AutoKerasWrapper.AutoKerasWrapper()

        elif self.library_name == "autogluon":
            self.library = AutoGluonWrapper.AutoGluonWrapper()
    ...
    #---------------------------------------------------------------------------------------------#
    def train(self, data: Any, target_column: str, task_type: Optional[str] = None, data_type: Optional[str] = None, problem_type: Optional[str] = None, hyperparameters: Optional[Dict[str, Any]] = None) -> None:
        if not self.is_initialized:
            self.initialize(task_type, data_type, problem_type)

        self.library.train_model(data, target_column, hyperparameters)
